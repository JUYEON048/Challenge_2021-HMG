#start point is based virtual env x,y
...
class Localization():
	...
	def subscribe(self):
		...
		ts = message_filters.ApproximateTimeSynchronizer([self.gps_data,self.imu_data,self.odom_data,self.udp_data], 2, 0.1, allow_headerless=True)
...
#		self.gps_set(gps)					# gps data loading
		self.val_set(imu, udp, odom)		# value set
