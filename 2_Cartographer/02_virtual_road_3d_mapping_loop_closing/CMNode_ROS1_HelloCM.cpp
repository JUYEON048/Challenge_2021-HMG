...
static struct {
    ...
    struct {
        ...
        struct {
            ...
            tRosIF_TpcPub<nav_msgs::Odometry> odom;
...
CMRosIF_CMNode_Init (int Argc, char **Argv, char *CMNodeName, struct tInfos *Inf)
{
	...
	strcpy(sbuf, "/odom");
    LOG("  -> Publish '%s'", sbuf);
    CMNode.Topics.Pub.odom.Pub         = node->advertise<nav_msgs::Odometry>(sbuf, static_cast<uint>(CMNode.Cfg.QueuePub));
    CMNode.Topics.Pub.odom.Job         = CMCRJob_Create("Odometry");
    CMNode.Topics.Pub.odom.CycleTime   = 50;
    CMNode.Topics.Pub.odom.CycleOffset = 0;
...
CMRosIF_CMNode_TestRun_Start_atBegin (struct tInfos *Inf)
{
	...
	*pmode = CMNode_Mode_Disabled;
	...
	job       = CMNode.Topics.Pub.odom.Job;
    cycletime = CMNode.Topics.Pub.odom.CycleTime;
    cycleoff  = CMNode.Topics.Pub.odom.CycleOffset;
...
CMRosIF_CMNode_Calc (double dt)
{
	...
	if (SimCore.State == SCState_Simulate) {
		...
		// Odometry
        if ((rv = CMCRJob_DoPrep(CMNode.Topics.Pub.odom.Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) < CMCRJob_RV_OK) {
            LogErrF(EC_Sim, "CMNode: Error on DoPrep for Job '%s'! rv=%s", CMCRJob_GetName(CMNode.Topics.Pub.odom.Job), CMCRJob_RVStr(rv));
        } else if (Lidar_CycleCount % (int)CMNode.Topics.Pub.odom.CycleTime == 0){
            tf2::Quaternion rotation;
            CMNode.Topics.Pub.odom.Msg.header.frame_id = "Fr0";
            CMNode.Topics.Pub.odom.Msg.header.stamp = ros::Time(SimCore.Time);
            //CMNode.Topics.Pub.Odometry.Msg.pose.covariance =
            //CMNode.Topics.Pub.Odometry.Msg.twist.covariance =
            CMNode.Topics.Pub.odom.Msg.pose.pose.position.x = InertialSensor[0].Pos_0[0];
            CMNode.Topics.Pub.odom.Msg.pose.pose.position.y = InertialSensor[0].Pos_0[1];
            CMNode.Topics.Pub.odom.Msg.pose.pose.position.z = InertialSensor[0].Pos_0[2];
		
            rotation.setRPY(Car.Roll, Car.Pitch, Car.Yaw);
            CMNode.Topics.Pub.odom.Msg.pose.pose.orientation = tf2::toMsg(rotation);
		
            CMNode.Topics.Pub.odom.Msg.child_frame_id = "Fr1A";
            CMNode.Topics.Pub.odom.Msg.twist.twist.linear.x = InertialSensor[0].Vel_0[0];
            CMNode.Topics.Pub.odom.Msg.twist.twist.linear.x = InertialSensor[0].Vel_0[1];
            CMNode.Topics.Pub.odom.Msg.twist.twist.linear.z = InertialSensor[0].Vel_0[2];
            CMNode.Topics.Pub.odom.Msg.twist.twist.angular.x = InertialSensor[0].Omega_0[0];
            CMNode.Topics.Pub.odom.Msg.twist.twist.angular.x = InertialSensor[0].Omega_0[1];
            CMNode.Topics.Pub.odom.Msg.twist.twist.angular.z = InertialSensor[0].Omega_0[2];
        }
        ...
        if ((rv = CMCRJob_DoPrep(CMNode.Topics.Pub.Imu.Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) < CMCRJob_RV_OK) {
			...
			//tf2::Transform::rotation = tf2::toMsg(rotation);
...
CMRosIF_CMNode_Out (void)
{
    ...
    if ((rv = CMCRJob_DoJob(out_imu->Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) != CMCRJob_RV_DoNothing
        ...
        //tf.transform.rotation = CMNode.Topics.Pub.UDP.Msg.orientation;
		tf2::Quaternion rotation;
        rotation.setRPY(0,0,0);

        tf.transform.rotation = tf2::toMsg(rotation);

        tf.transform.translation.x = 0; //2.5226
        tf.transform.translation.y = 0;
        tf.transform.translation.z = 0; //0.5424

        tf.header = out_imu -> Msg.header;
    ...
    auto out_Odometry = &CMNode.Topics.Pub.odom;
    ...
    if ((rv = CMCRJob_DoJob(out_Odometry->Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) != CMCRJob_RV_DoNothing
        ...
        geometry_msgs::TransformStamped tf;
        tf.transform.rotation = CMNode.Topics.Pub.odom.Msg.pose.pose.orientation;
        tf.transform.translation.x = CMNode.Topics.Pub.odom.Msg.pose.pose.position.x;
        tf.transform.translation.y = CMNode.Topics.Pub.odom.Msg.pose.pose.position.y;
        tf.transform.translation.z = CMNode.Topics.Pub.odom.Msg.pose.pose.position.z;
        tf.header = CMNode.Topics.Pub.odom.Msg.header;
        tf.child_frame_id = CMNode.Topics.Pub.odom.Msg.child_frame_id;
        CMNode.TF.br->sendTransform(tf);
